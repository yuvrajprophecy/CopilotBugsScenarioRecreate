{{
  config({    
    "materialized": "table",
    "alias": "op_sql_1k_lu_document_selection_disabled",
    "database": "main",
    "schema": "rishabh"
  })
}}

WITH `1k_aka_gpdip_edlud_312` AS (
...
Join_310_inner AS (
  ...
  FROM {{ source('prophecy__temp_OP_LU_Document_Selection_Disabled_source', 'prophecy__temp_OP_LU_Document_Selection_Disabled_pre_Join_299_inner_0') }}
...
Join_318_inner AS (
  ...
  FROM {{ source('prophecy__temp_OP_LU_Document_Selection_Disabled_source', 'prophecy__temp_OP_LU_Document_Selection_Disabled_pre_Join_313_inner_0') }}
