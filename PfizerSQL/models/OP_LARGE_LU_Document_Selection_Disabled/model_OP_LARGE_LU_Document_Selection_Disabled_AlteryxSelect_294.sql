...
WITH Join_318_inner AS (
  ...
  FROM {{ source('prophecy__temp_OP_LARGE_LU_Document_Selection_Disabled_source', 'prophecy__temp_OP_LARGE_LU_Document_Selection_Disabled_pre_Join_313_inner_0') }}

),

`1m_aka_gpdip_edlud_312` AS (

  SELECT * 
  
  FROM {{ source('main.rishabh', '1m_aka_gpdip_edlud_312') }}
...
Join_313_inner AS (
  ...
  INNER JOIN `1m_aka_gpdip_edlud_312` AS in1
...
`1m_aka_gpdip_edlud_298` AS (

  SELECT * 
  
  FROM {{ source('main.rishabh', '1m_aka_gpdip_edlud_298') }}
...
Join_310_inner AS (
  ...
  FROM {{ source('prophecy__temp_OP_LARGE_LU_Document_Selection_Disabled_source', 'prophecy__temp_OP_LARGE_LU_Document_Selection_Disabled_pre_Join_299_inner_0') }}
...
Join_299_inner AS (
  ...
  INNER JOIN `1m_aka_gpdip_edlud_298` AS in1
     ON in0.r_object_id = in1.r_object_id
