{{
  config({    
    "materialized": "table",
    "alias": "prophecy__temp_test_pipeline_post_AlteryxSelect_294_0",
    "database": "main",
    "schema": "rishabh"
  })
}}
...
WITH aka_GPDIP_EDLUD_298 AS (
  ...
  FROM {{ source('prophecy__temp_test_pipeline_source', 'prophecy__temp_test_pipeline_pre_Join_299_inner_0') }}
...
Join_310_inner AS (
  ...
  FROM {{ source('prophecy__temp_test_pipeline_source', 'prophecy__temp_test_pipeline_pre_Join_299_inner_1') }}
...
Join_318_inner AS (
  ...
  FROM {{ source('prophecy__temp_test_pipeline_source', 'prophecy__temp_test_pipeline_pre_Join_313_inner_1') }}
...
aka_GPDIP_EDLUD_312 AS (
  ...
  FROM {{ source('prophecy__temp_test_pipeline_source', 'prophecy__temp_test_pipeline_pre_Join_313_inner_0') }}
