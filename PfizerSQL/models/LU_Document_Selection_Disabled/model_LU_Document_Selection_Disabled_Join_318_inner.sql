...
WITH `1k_aka_gpdip_edlud_324` AS (

  SELECT * 
  
  FROM {{ source('main.rishabh', '1k_aka_gpdip_edlud_324') }}
...
Filter_323 AS (
  ...
  FROM `1k_aka_gpdip_edlud_324` AS in0
...
Cleanse_290 AS (
  ...
  FROM {{ source('prophecy__temp_LU_Document_Selection_Disabled_source', 'prophecy__temp_LU_Document_Selection_Disabled_post_Cleanse_290_0') }}
